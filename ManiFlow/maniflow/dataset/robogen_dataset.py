...
from maniflow.common.sampler import (get_val_mask, downsample_mask)
...
from maniflow.dataset.Augmentations.aug_translation_xy import TranslationXY
from maniflow.dataset.Augmentations.aug_rotation_z import rotationZ
from maniflow.dataset.Augmentations.random_apply_numpy import RandomApplyNumpy
...
from maniflow.dataset.robogen_utils import gripper_pcd_to_10d_vector, rotation_transfer_6D_to_matrix_batch, rotation_transfer_matrix_to_6D_batch
from maniflow.dataset.robogen_all_data import *
...
def get_zarry_paths(zarr_path):
    ...
    if zarr_path == 'test':
        dataset_prefix = '/data/robogen/sim_chenyuan/ManiFlow_Policy/ManiFlow/data/top_test'
        data_name = [os.listdir(dataset_prefix)[0]]
        all_zarr_paths = [
            "{}/{}".format(dataset_prefix, data_name[i]) for i in range(len(data_name))
        ]
    if zarr_path == 'test_orchard':
    ...
    if zarr_path == 'grasp':
        grasp_prefix = '/tmp/dp3_demo_combined_2_step_0/grasp'
    ...
    if zarr_path == 'grasp_lift':
        grasp_prefix = '/tmp/dp3_demo_combined_2_step_0/grasp_lift'
        grasp_names = os.listdir(grasp_prefix)
        grasp_zarr_paths = [
            "{}/{}".format(grasp_prefix, grasp_names[i]) for i in range(len(grasp_names))
        ]
        all_zarr_paths = grasp_zarr_paths
    if zarr_path == 'close_only':
    ...
    if zarr_path == 'pick_and_place_only':
        top_prefix = '/tmp/dp3_demo_combined_2_step_0/top'
        ...
        inside_link_prefix = '/tmp/dp3_demo_combined_2_step_0/inside_link'
...
class RobogenDataset(BaseDataset):
    def __init__(self,
        ...
        if not self.kept_in_disk:
            from maniflow.common.sampler import SequenceSampler
            ...
            from maniflow.common.sampler_disk import SequenceSampler
            self.sampler = SequenceSampler(
                replay_buffer=self.replay_buffer, 
                sequence_length=horizon,
                pad_before=pad_before, 
    ...
    def _sample_to_data(self, sample):
        ...
        task_name = all_task_names[int(cat_idx[0])]
