...
from maniflow.model.diffusion.conv1d_components import (
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        if language_conditioned:
            self.language_encoder_out_dim = self.load_T5_encoder()
            cond_dim += self.language_encoder_out_dim
    ...
    def load_T5_encoder(self, freeze=True):
        ...
        T5_language_encoder_out_dim = 512
        cprint(f"Loaded T5 encoder: {encoder_name}", "green")
        return T5_language_encoder_out_dim
    ...
    def forward(self, 
        ...
        language_goal: (B,) or string, language condition, optional
        ...
        if global_cond is not None:
            ...
            global_feature = torch.cat([timestep_embed, global_cond], axis=-1) # [B, D] or [B, T, D]
        
        if self.language_conditioned:
            assert lang_cond is not None, "Language condition is required"
            assert global_feature is not None, "Global condition is required"
            lang_embedding = self.encode_text_input_T5(lang_cond, 
                                                       norm_lang_embedding=True,
                                                       device=sample.device)
            global_feature = torch.cat([global_feature, lang_embedding], axis=-1) # [B, D]
...
class ConditionalUnet1D_distance(ConditionalUnet1D):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)        
        dsed = self.dsed
        self.diffusion_step_encoder = nn.Sequential(
            SinusoidalPosEmb(self.dsed),
            nn.Linear(self.dsed, self.dsed * 4),
            nn.Mish(),
            nn.Linear(self.dsed * 4, dsed),
        )
        self.diffusion_distance_encoder = nn.Sequential(
            SinusoidalPosEmb(dsed),
            nn.Linear(dsed, dsed * 4),
            nn.Mish(),
            nn.Linear(dsed * 4, dsed),
        ...
        self.adpat_step_distance_dim = nn.Linear(dsed * 2, dsed)
    ...
    def forward(self, 
            sample: torch.Tensor, 
            timestep: Union[torch.Tensor, float, int], 
            distance: Union[torch.Tensor, float, int],
            local_cond=None, global_cond=None, 
            language_goal=None,
            **kwargs):
        """
        ...
        if not torch.is_tensor(distances): 
            distances = torch.tensor([distances], dtype=torch.float32, device=sample.device)    
        elif torch.is_tensor(distances) and len(distances.shape) == 0:
            distances = distances[None].to(sample.device)
        distances = distances.expand(sample.shape[0])
        distance_embed = self.diffusion_distance_encoder(distances) # (B, n_emb)

        # 3. adapt step and distance
        step_distance_embed = torch.cat([timestep_embed, distance_embed], dim=-1)
        step_distance_embed = self.adpat_step_distance_dim(step_distance_embed)

        if global_cond is not None:
            if self.condition_type == 'cross_attention':
                step_distance_embed = step_distance_embed.unsqueeze(1).expand(-1, global_cond.shape[1], -1)
            global_feature = torch.cat([step_distance_embed, global_cond], axis=-1) # [B, D] or [B, T, D]
        
        if self.language_conditioned:
            assert language_goal is not None, "Language condition is required"
            assert global_feature is not None, "Global condition is required"
            lang_embedding = self.encode_text_input_T5(language_goal, 
                                                       norm_lang_embedding=True,
                                                       device=sample.device)
            global_feature = torch.cat([global_feature, lang_embedding], axis=-1) # [B, D]
...
if __name__ == '__main__':
    sample = torch.randn(2, 100, 32)
    timestep = torch.tensor([1, 2])
    local_cond = None
    global_cond = torch.randn(2, 256)
    model = ConditionalUnet1D(32, 256, 256)
