...
from maniflow.gym_util.mjpc_diffusion_wrapper import MujocoPointcloudWrapperAdroit
...
class AdroitRunner(BaseRunner):
    def __init__(self,
        ...
        if action_length_dict is not None:
            cprint(f"action_length_dict: {action_length_dict}", 'green')
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Adroit {self.task_name} Pointcloud Env",
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    # clip action based on action_length_dict if it is provided
                    if self.action_length_dict is not None:
                        task_action_length = self.action_length_dict[self.task_name]
                        action_dict['action'] = action_dict['action'][..., :task_action_length]
