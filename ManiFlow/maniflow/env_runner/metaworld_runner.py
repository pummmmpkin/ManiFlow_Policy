...
import collections
import tqdm
from maniflow.env import MetaWorldEnv
...
from maniflow.common.model_util import ForkedPdb
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Metaworld {self.task_name} Pointcloud Env", leave=False, mininterval=self.tqdm_interval_sec):
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    obs_dict_input['point_cloud'] = obs_dict['point_cloud'].unsqueeze(0).to(policy.device)
                    obs_dict_input['agent_pos'] = obs_dict['agent_pos'].unsqueeze(0).to(policy.device)
                    obs_dict_input['task_name'] = [self.task_name]
                    # assert policy device is the same as obs device
                    assert obs_dict_input['point_cloud'].device == policy.device, f"policy device: {policy.device}, obs device: {obs_dict_input['point_cloud'].device}"
