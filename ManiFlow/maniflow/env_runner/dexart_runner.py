...
from maniflow.env import DexArtEnv
...
class DexArtRunner(BaseRunner):
    def __init__(self,
                 ...
                 task_name=None,
                 use_test_env=False,
                 agent_pos_length=None, # 32, to align for multitasks
                 ):
        ...
        self.env_train = env_fn(is_test=use_test_env)
        ...
        self.agent_pos_length = agent_pos_length

        self.logger_util_train = logger_util.LargestKRecorder(K=3)
        self.logger_util_train10 = logger_util.LargestKRecorder(K=5)

        if self.agent_pos_length is not None:
            cprint( f"Using fixed agent_pos_length: {self.agent_pos_length}", 'yellow')
        ...
        for episode_id in tqdm.tqdm(range(self.episode_train), desc=f"DexArt {self.task_name} Train Env",leave=False, mininterval=self.tqdm_interval_sec):
            ...
            for step_id in range(self.max_steps):
                ...
                with torch.no_grad():
                    ...
                    if self.agent_pos_length is not None:
                        # pad to fixed length, [B, L] to [B, agent_pos_length]
                        padding = torch.zeros(obs_dict['agent_pos'].shape[0], self.agent_pos_length - obs_dict['agent_pos'].shape[1]).to(device)
                        obs_dict_input['agent_pos'] = torch.cat([obs_dict['agent_pos'], padding], dim=1).unsqueeze(0)
                    else:
