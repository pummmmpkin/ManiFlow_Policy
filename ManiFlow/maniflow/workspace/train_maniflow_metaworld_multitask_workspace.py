...
from maniflow.policy.maniflow_pointcloud_policy import ManiFlowTransformerPointcloudPolicy as ManiFlow
from maniflow.dataset.base_dataset import BaseDataset
from maniflow.env_runner.base_runner import BaseRunner
from maniflow.env_runner.metaworld_multitask_runner import MetaworldMultitaskRunner
...
class TrainManiFlowMetaWorldMultiTaskWorkspace:
    ...
    def run(self):
        ...
        env_runner: MetaworldMultitaskRunner = hydra.utils.instantiate(
            cfg.task.env_runner,
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            if rollout_multiple_inference_step:
                ...
                if (self.epoch % cfg.training.rollout_every) == 0 and RUN_ROLLOUT and env_runner is not None and self.epoch > 1: #  and self.epoch > 1
                    t3 = time.time()
                    for inference_step in all_rollout_steps:
                        ...
                        mean_metrics = {}
                        for metric in runner_log[cfg.task.dataset.task_names[0]].keys():
                            # Skip the video
                            if metric in ['sim_video_eval']:
                                continue
                            values = [runner_log[task][metric] for task in cfg.task.dataset.task_names]
            ...
            if (self.epoch % cfg.training.sample_every) == 0:
                with torch.no_grad():
                    ...
                    # print action prediction error
                    if (self.epoch % (cfg.training.sample_every * 5)) == 0:
                        cprint(f"Epoch {self.epoch} - Train Action MSE Error: {mse.item():.4f}", 'green')
                    del batch
                    del obs_dict
                    del gt_action
                    del result
...
def main(cfg):
    workspace = TrainManiFlowMetaWorldMultiTaskWorkspace(cfg)
    workspace.run()
